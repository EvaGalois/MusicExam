...
class musicPlayer(QtWidgets.QMainWindow):
    ...
    def Event(self):
        ...
        #self.qlist.activated.connect(lambda:self.waveplot(2))
        self.qlist.activated.connect(self.waveplot)
        #self.qlist.activated.connect(self.waveplotstart)#用这个加上去掉颜色的原版代码可以运行
    ...
    def layouts(self):
        ...
        self.setStyleSheet('''
            ...
            QLabel {color: white;}
            QPushButton {border-radius: 6; height: 30; border-radius: 15; color: white;}
            QPushButton:hover {background: rgba(90,160,210,.6); color: black;}
    ...
    def waveplotstart(self):
        self.waveplot(classnum=2)
    ...
    def waveplot(self, classnum=2):
        ...
        librosa.display.waveplot(self.re_audio, self.re_sample, linewidth=.1, max_sr=10)#, max_sr=10, color='#8ef'
        plt.title('signal')
        plt.xlabel('Time')
        ...
        plt.bar(np.arange(len(self.chroma_cqt_bar_normalized)), self.chroma_cqt_bar_normalized, alpha=1)#, color='#6ac', alpha=1
    ...
    def calculus(self): #相似度计算，算的是余弦相似度。
        # print(self.chroma_cqt.shape)
